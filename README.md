...
1. **+ Device** — type brand, model, U height and a connector list. The panel
   drawing is generated by `autoLayout()`. Fastest path, decent results.
2. **Find specs** — the app is offline and can't search the web, so this gives
   ...
   record; paste it back in. Ask it to cite the datasheet it used.
3. **Edit `devices.js`** — hand-place elements with explicit `x`/`y` for the
   devices you care most about. `ah-ar2412` and `db-d20` are the worked examples:
   both were drawn from the manufacturer's own orthographic front view, which is
   the standard the rest of the library is held to.

There are also two generators for faces that repeat: `switchFront()` for network
switches (see the NetBox importer) and `mixrackFront()` for control-less boxes
that are just vents and lettering.

Devices added through 1 and 2 live in the project file, so they travel with a
saved `.json`. Devices in `devices.js` are shared across all projects.
